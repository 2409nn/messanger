<script setup>
  ...
  const callingSound = new Audio(callSound);
  const callEndedSound = new Audio(callEnded);
  ...
  const isCameraActive = ref(props.isVideo);
  ...
  watch(() => props.isVideo, (newVal) => {
    isCameraActive.value = newVal;
  })
...
<div class="callWindow" :class="{'active': isActive}">
  ...
  <div class="callWindow__buttons">
    <button class="callWindow__button callWindow__button-microphone" @click="isMicrophoneActive = !isMicrophoneActive">
      <svg class="callWindow__button-icon" width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect :class="{'active': !isMicrophoneActive}" class="callWindow__svg-cross" id="microphone-cross" x="4.62529" y="2.66476" width="1.69274" height="16.4752" rx="0.59637" transform="rotate(-39.2597 4.62529 4.66476)" fill="currentColor" stroke="currentColor" stroke-width="0.5"/>
      </svg>
    </button>
    <button class="callWindow__button callWindow__button-camera" @click="isCameraActive = !isCameraActive">
      <svg class="callWindow__button-icon" width="30" height="30" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <rect :class="{'active': !isCameraActive}" class="callWindow__svg-cross" id="camera-cross" x="0.351781" y="3.10659" width="1.69274" height="20.5"  rx="0.59637" transform="rotate(-39.2597 0.351781 3.10659)" fill="currentColor" stroke="currentColor" stroke-width="0.5"/>
      </svg>
    </button>
